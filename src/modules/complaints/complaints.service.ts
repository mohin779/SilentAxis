...
export async function submitComplaint(input: {
  ...
  if (!(await zkVerifier.verifyProof(input.proof, input.root, input.nullifierHash))) {
  ...
  const existing = await pool.query("SELECT 1 FROM nullifiers WHERE nullifier_hash = $1", [
    input.nullifierHash
  ]);
  ...
  await pool.query("INSERT INTO nullifiers (nullifier_hash) VALUES ($1)", [input.nullifierHash]);
  await pool.query(
